"""
Parameter sweeper for training multiple model configurations and comparing their performance.
Automatically trains models using e_Train.py with different hyperparameter configurations,
saving each configuration to document the sweep.
"""
...
# ===========================================================================================
# PARAMETER SWEEP SUPPORT FUNCTIONS
# ===========================================================================================

def load_base_config(config_path):
    """Load base configuration file (YAML or JSON)."""
...
def train_with_config(config_path, verbose=True):
    """
    Train a model using e_Train.py with the given config file.
    ...
    try:
        result = subprocess.run(
            [sys.executable, "e_Train.py", "--config", str(config_path)],
            capture_output=True,
            text=True,
        ...
        print(f"Training timeout for config: {config_path}")
        return False
    except Exception as e:
        print(f"Training failed with error: {e}")
        return False

def load_trained_model(config, data_dir, forecast_name, model_name):
    """
    Load the trained model based on config and model paths.
    
    Returns:
    - model: Loaded model
    - model_type: Type of model (transformer, xgb_regressor, xgb_classifier)
    """
...
if __name__ == '__main__':
    ...
    sweep_dir = Path(data_dir, "sweep_results")
    ...
    print(f"\nLoading base configuration from: {base_config_path}")
    base_config = load_base_config(base_config_path)
    
    # Extract model metadata from base config
    input_columns = base_config["data"]["input_columns"]
    output_columns = base_config["data"]["output_columns"]
    input_rows = slice(base_config["data"]["input_row_1"], base_config["data"]["input_row_2"])
    output_rows = base_config["data"]["output_rows"]
    ...
    for iteration, values in enumerate(product(*param_values)):
        print(f"\n{'='*80}")
        print(f"SWEEP ITERATION {iteration + 1}")
        print(f"{'='*80}")
        
        # Create iteration-specific directory
        iter_dir = sweep_dir / f"iteration_{iteration:03d}"
        iter_dir.mkdir(parents=True, exist_ok=True)
        
        # Copy and modify config for this iteration
        iter_config = json.loads(json.dumps(base_config))  # Deep copy
        ...
        # Update model name to include iteration
        iter_model_name = f"{base_model_name}_iter{iteration:03d}"
        iter_config["model_name"] = iter_model_name
        
        # Save iteration config
        iter_config_path = iter_dir / "config.yaml"
        save_config(iter_config, iter_config_path)
        print(f"  Config saved to: {iter_config_path}")
        
        # Train model with this configuration
        print(f"\n  Training model...")
        success = train_with_config(iter_config_path, verbose=True)
        
        if not success:
            print(f"  WARNING: Training failed for iteration {iteration}")
            continue
        
        # Load trained model and config
        try:
            model, model_type = load_trained_model(iter_config, data_dir, forecast_name, iter_model_name)
            
            # Load model config to get input/output dimensions
            with open(Path(data_dir, 'forecasts', forecast_name, iter_model_name, 'model_config.json'), 'r') as f:
                model_config = json.load(f)
    ...
    print(f"\n{'='*80}")
    print(f"COMPLETED {len(sweep_configs)} SUCCESSFUL ITERATIONS")
    print(f"{'='*80}")

    ##################################################################################################################
    ## EVALUATE SWEPT MODELS
    
    if not sweep_configs:
        print("No successful sweep iterations. Exiting.")
        sys.exit(0)
    
    print(f"\n{'='*80}")
    print("EVALUATING SWEPT MODELS")
    print(f"{'='*80}")
    ...
    for config_entry in sweep_configs:
        ...
        model_type = config_entry["model_type"]
        model = config_entry["model"]
        parameters = config_entry["parameters"]
        
        print(f"\nEvaluating iteration {iteration}: {model_name}")
        
        try:
            if model_type == "transformer":
                preds, targets = evaluate_model(model, test_dataset)
            elif model_type == "xgb_regressor":
            ...
            swept_alternatives.append((preds, targets))
            param_str = ", ".join([f"{k.split('.')[-1]}={v}" for k, v in parameters.items()])
            swept_labels.append(f"Iter {iteration} ({param_str})")
    ...
    all_alternatives = swept_alternatives + [
        (naive_preds, naive_targets),
        (linear_preds, linear_targets),
        (seasonal_preds, seasonal_targets)
    ]
    all_labels = swept_labels + ["Naive", "Linear", "Seasonal"]

    ################################################################################################################
    ## Visualize results
    print(f"\nGenerating visualizations...")
    visualizer(*all_alternatives, labels=all_labels, forecast_name=forecast_name, directory=data_dir, num_samples=200)
    
    # Save sweep summary to CSV
    summary_data = []
    for i, (preds, targets) in enumerate(all_alternatives):
    ...
    summary_csv = Path(sweep_dir, "sweep_summary.csv")
