...
def prepare_separate_analysis(raw_df, sensor_name):
    ...
    for point_num, corr_col in enumerate([('Correction1', 'Standard', 'Post Calibration Value'),
        ...
        df = raw_df[raw_df[corr_col_name].notna()].copy()
        if len(df) < 1:
            continue
        
        # Extract numeric correction values
        df['Error'] = pd.to_numeric(df[corr_col_name], errors='coerce')
...
def create_error_distribution_visualizations(og_df, sensor_name, output_dir=OUTPUT_DIR):
    ...
    Shows offset, gain, and noise histograms, time series, Q-Q plots, and correlations.
    """
    if len(og_df) < 2:
        return
    ...
    if len(offsets) > 2:
        ...
        ax.set_title('Offset Q-Q Plot', fontweight='bold', fontsize=12)
        ax.grid(True, alpha=0.25)
        ax.set_axisbelow(True)
    
    # Offset Time Series (removed: Event Index not meaningful)
    ...
    if len(gains) > 2:
        ...
        ax.set_title('Gain Q-Q Plot', fontweight='bold', fontsize=12)
        ax.grid(True, alpha=0.25)
        ax.set_axisbelow(True)
    else:
        ax.axis('off')
    
    # Gain Time Series (removed: Event Index not meaningful)
    ...
    if len(noise_vars) > 2:
        ...
        ax.set_title('Noise Variance Q-Q Plot', fontweight='bold', fontsize=12)
        ax.grid(True, alpha=0.25)
        ax.set_axisbelow(True)
    
    # Noise Time Series (removed: Event Index not meaningful)
...
def create_offset_gain_correlations(og_df, sensor_name, output_dir=OUTPUT_DIR):
    ...
    # Row 2: Gain and Noise correlations
...
def create_cross_sensor_significance_summary(all_stats, output_dir=OUTPUT_DIR):
    """Create cross-sensor summary of significant predictor effects and model fit."""
    if not all_stats:
        return
    ...
    predictor_keys = [
        ('Offset vs Temp', 'Offset_vs_Temperature_significant'),
        ('Gain vs Temp', 'Gain_vs_Temperature_significant'),
        ('Noise vs Temp', 'Noise_vs_Temperature_significant'),
        ('Offset vs Time', 'Offset_vs_Timespan_significant'),
        ('Gain vs Time', 'Gain_vs_Timespan_significant'),
        ('Noise vs Time', 'Noise_vs_Timespan_significant'),
    ]

    sig_matrix = np.full((len(sensors), len(predictor_keys)), np.nan)
    for i, row in stats_df.reset_index(drop=True).iterrows():
        for j, (_, key) in enumerate(predictor_keys):
            if key in row and pd.notna(row[key]):
                sig_matrix[i, j] = 1 if bool(row[key]) else 0

    fig, axes = plt.subplots(1, 2, figsize=(18, 8), gridspec_kw={'width_ratios': [2.5, 1]})
    fig.suptitle('Cross-Sensor Significance Summary', fontsize=14, fontweight='bold', y=0.995)

    # Predictor significance heatmap
    ax = axes[0]
    masked = np.ma.masked_invalid(sig_matrix)
    ax.imshow(masked, cmap='RdYlGn', vmin=0, vmax=1, aspect='auto')
    ...
    ax.set_title('Predictor Significance (p < 0.05)', fontweight='bold', fontsize=12)
    for i in range(len(sensors)):
        for j in range(len(predictor_keys)):
            if np.isnan(sig_matrix[i, j]):
                text = '-'
            else:
                text = 'Y' if sig_matrix[i, j] == 1 else 'N'
            ax.text(j, i, text, ha='center', va='center', color='black', fontsize=11, fontweight='bold')

    # Model fit significance rate
    ax = axes[1]
    if 'Model_Significance_Rate' in stats_df.columns:
        model_rates = stats_df['Model_Significance_Rate'].fillna(0) * 100
        ax.barh(sensors, model_rates, color='#1f77b4', alpha=0.75, edgecolor='black', linewidth=1.0)
        ax.set_xlabel('Significant Model Fit (%)', fontweight='bold')
        ax.set_title('Model Fit Significance', fontweight='bold', fontsize=12)
        ax.grid(True, alpha=0.25, axis='x')
        ax.set_axisbelow(True)
        ax.invert_yaxis()
    else:
...
def create_offset_gain_visualizations(og_df, og_stats, sensor_name, output_dir=OUTPUT_DIR):
    ...
    # Row 1: Offset
    # Offset over time (removed: Event Index not meaningful)
    ax = axes[0, 0]
    ax.axis('off')
    ...
    # Row 2: Gain (multi-point only)
    multi_point_df = og_df[og_df['Fit_Type'] == 'Multi_Point']
    
    # Gain over time (removed: Event Index not meaningful)
    ax = axes[1, 0]
    ax.axis('off')
