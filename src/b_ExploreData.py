...
import pandas as pd
from utils.names import label as names_label
...
def _norm_surface_sensor_key(s: str) -> str:
    """Normalize a surface sensor name to a canonical key for matching.
    Handles µ/micro variants, unicode punctuation, and whitespace.
    """
    s = str(s).lower().replace("\u00b5", "micro").replace("\u03bc", "micro")
...
def _load_surface_calibration_times(repo_root: Path) -> dict:
    """
    Load per-sensor calibration end times from per-sensor summary CSVs under
    data/output/calibration/summaries/*.csv.
    Returns mapping: normalized_sensor_name -> sorted list of pd.Timestamp.
    """
    summaries_root = repo_root / "data" / "output" / "calibration" / "summaries"
    if not summaries_root.exists():
        return {}
    result = {}
    for csv_path in summaries_root.glob("*.csv"):
        sensor_key = _norm_surface_sensor_key(csv_path.stem)
        if not sensor_key:
            continue
        try:
            cal_df = pd.read_csv(csv_path, low_memory=False)
            if "Calibration End Time" not in cal_df.columns:
                continue
            times = pd.to_datetime(cal_df["Calibration End Time"], errors="coerce").dropna()
            if times.empty:
                continue
            result[sensor_key] = sorted(times.tolist())
        except Exception:
            continue
    print(f"[CALIBRATION] Loaded calibration times for {len(result)} sensors.")
    return result
...
def write_category_timeseries_columns(repo_root: Path) -> None:
    ...
    surface_calibration_times = _load_surface_calibration_times(repo_root)
    limits_records = load_limits_records(limits_csv)
    all_limit_specs = map_limits_to_columns(all_cols, limits_records)
    ...
    # Excluded against the *full* target and delta lists, as in the split-comparison section
    # above.  Using the ordered lists instead would let 14 lab columns through: ``_res`` and
    # ``_diff`` share a base name, so ``target_diff_by_base`` keeps only one column per base
    # and the other of each pair falls into the predictor set, roughly doubling the count.
    predictor_cols_for_split = [c for c in all_cols if c not in target_cols and c not in target_diff_cols]
    window = 168
    first_valid_idx = window - 1
    ...
    for figure_name, cols in category_columns.items():
        if not cols:
            continue
        # Row labels come from the shared registry rather than by stripping characters off
        # the stored column name.  Each of these figures draws a single dataset, so the
        # source qualifier would be constant down the whole column and is omitted; the
        # figure caption names the dataset.  These panels plot measured values, so the
        # units are kept.  Target_diff plots the change for every row, which the caption
        # states once, so the per-row suffix is dropped.
        keep_suffix = figure_name != "Target_diff"
        labels = [
    ...
    row_height = 0.88
    min_fig_height = 2.8
    # These figures are drawn 13 in wide and then scaled to the 6.5 in text block, so
    # everything on the page is half the size set here.  At the previous font_size = 10
    # the y tick values printed at 5 pt.  Sizes are now chosen so that the smallest text
    # lands at ~7 pt after that 2x reduction.
    font_size = 14
    ...
    for figure_name, cols in category_columns.items():
        ...
        for variant_suffix, include_surface_uncertainty in variant_specs:
            ...
            for i, (ax, col, label) in enumerate(zip(axes, cols, wrapped_labels)):
                ...
                if figure_name in {"Target", "Target_diff"}:
                    ...
                    if figure_name == "Surface":
                        ...
                        for cal_ts in surface_calibration_times.get(cal_key, []):
                            if start_date <= cal_ts <= end_date:
                                ax.axvline(
                                    x=cal_ts,
                                    color="#e31a1c",
                                    linewidth=0.9,
            ...
            if _add_raster:
                ...
                _cb.ax.tick_params(labelsize=font_size)
            ...
            if figure_name == "Surface":
                ...
                if "uncertainty" in variant_suffix:
                    ...
                    sigma1_handle = plt.Line2D([0], [0], color="black", linewidth=0.8, linestyle="--", label="±1σ boundary")
                    sigma2_handle = plt.Line2D([0], [0], color="black", linewidth=0.6, linestyle=":", label="±2σ boundary")
